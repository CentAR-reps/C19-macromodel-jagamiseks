...
output: 
  bookdown::html_document2:
    ...
    toc_float: true
    theme: cosmo
bibliography: references_valisnoudlus.bib
link-citations: yes
linkcolor: blue
editor_options: 
  markdown: 
...
Kuna antud andmestik kajastab aastast ekspordimahtu, siis kuiste simulatsioonide korral jagatakse vastavad väärtused läbi 12-ga. Sellisest lähenemisest tulenevalt ei ole võimalik arvesse võtta ekspordi sesoonsust.
...
Kasutaja saab sisendfailis lehel *topK* iga majandusharu $k$ kõige olulisema sihtturu (ja lisaks *ROW*) jaoks määrata, mil määral nõudlus suureneb/väheneb. Seejuures veergudena saab käsitleda muutust üle kuude (n-ö aegreana) kui ka stsenaariume eraldiseisvalt.  
...
- *riik* korral toimub kõikide sihtriikide muutuste määramine lehel *countries*. Seejuures ridades on riigid ja veergudes kuud (aegrea korral) või stsenaariumid. 
- *riik-majandusharu* korral on iga riigi jaoks loodud eraldi leht. Igal lehel on ridades majandusharud ja veergudes kuud (aegrea korral) või stsenaariumid.
...
2. absoluutseid muutuseid (*absolute_change*)
3. absoluuttasemeid (*absolute_level*)

Seejuures peavad sisendid olema sama tüüpi üle sisendfaili, st nii topK kui sihtturu põhiste väärtuste tüübid on samad. Seda, millisele tüübile sisendfail vastab, määrab kasutaja ekspordi mooduli jooksutamisel.
...
- $X_{i,t}$ tähistab majandusharu $i$ ekspordi baastaset kuul $t$, 
...
*Märkus: kuud $t$ eristatakse juhul, kui sisendit väljendatakse aegreana. Stsenaariumite korral ajalist mõõdet ei esine. See kehtib ka järgnevate valemite korral.*
...
- $\tilde{X_{i,t}^j}$ tähistab majandusharu $i$ ekspordimahtu sihtturule $j$ kuul $t$, 
- $N$ on sihtturgude arv,
...
Ekspordi mooduli väljund on laial kujul andmetabel, milles ridades on sisemaised majandusharud ja veergudes kuud aegrea korral või stsenaariumid. Väärtusteks on ekspordi mahu muutuste hinnangud absoluutsummas (eurodes):

$$\Delta_{i,t} = \hat{X_{i,t}} - X_{i,t},$$

- $X_{i,t}$ tähistab majandusharu $i$ ekspordi baastaset kuul $t$, 
- $\hat{X_{i,t}}$ tähistab majandusharu $i$ simuleeritud ekspordi taset kuul $t$,
- $\Delta_{i,t}$ tähistab majandusharu $i$ ekspordimahu absoluutsummas muutust kuul $t$ (baastasemega võrreldes).
